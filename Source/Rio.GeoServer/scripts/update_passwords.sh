...
# for each datastore file, set my database server, my db username and my db password to it
DATASTORE_XML=${DATASTORE_XML:-/opt/geoserver/data_dir/workspaces/Rio/GeoServerSqlDataSource/datastore.xml}
cp $DATASTORE_XML $DATASTORE_XML.orig
cat $DATASTORE_XML.orig | sed -e "s/@database_server_name@/${DATABASE_SERVER_NAME}/" | sed -e "s/@database_user_name@/${DATABASE_USER_NAME}/" | sed -e "s/@database_user_password@/${DATABASE_USER_PASSWORD}/" > $DATASTORE_XML
