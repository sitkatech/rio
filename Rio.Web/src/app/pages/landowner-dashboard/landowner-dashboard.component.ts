...
import { TradeStatusEnum } from "src/app/shared/generated/enum/trade-status-enum";
...
import {
    MultiSeriesEntry,
    SeriesEntry,
} from "src/app/shared/models/series-entry";
...
export class LandownerDashboardComponent implements OnInit, OnDestroy {
    @ViewChild("landownerWaterUseChart")
    landownerWaterUseChart: LandownerWaterUseChartComponent;
    ...
    public sectionCurrentlyViewing: LandownerDashboardViewEnum =
        LandownerDashboardViewEnum.WaterBudget;
    ...
    set highlightedParcelID(value: number) {
        ...
        this.highlightedParcelDto = this.parcels.filter(
            (x) => x.ParcelID == value
        )[0];
    }
    ...
    public emptyCumulativeWaterUsage: SeriesEntry[] = this.months.map((y) => {
        return this.createSeriesEntry(y, 0);
    });
    public waterTypes: WaterTypeDto[];

    public selectedParcelsLayerName: string =
        "<img src='./assets/main/images/parcel_blue.png' style='height:16px; margin-bottom:3px'> Account Parcels";
    ...
    ngOnInit() {
        ...
        this.authenticationService.getCurrentUser().subscribe((currentUser) => {
            ...
            this.tradeStatusIDs = [
                ...
                TradeStatusEnum.Countered,
                TradeStatusEnum.Rejected,
                TradeStatusEnum.Rescinded,
            ];
            this.postingStatusIDs = [
                PostingStatusEnum.Open,
                PostingStatusEnum.Closed,
            ];
    ...
    public getViewEnum(): string[] {
        ...
        if (!this.allowTrading()) {
            values = values.filter(
                (x) => x != LandownerDashboardViewEnum.Trading
            );
        }
    ...
    public getImgSrcForSelector(value: string): string {
        var color =
            this.checkSelectedView(value) || this.hoverItem == value
                ? "white"
                : "black";
    ...
    public updateView(value: string) {
        var key = Object.keys(LandownerDashboardViewEnum).filter(
            (x) => LandownerDashboardViewEnum[x] == value
        )[0];
    ...
    public updateAccountData(account: AccountSimpleDto): void {
        forkJoin({
            postings: this.postingService.getPostingsByAccountID(
                account.AccountID
            ),
            trades: this.tradeService.getTradeActivityByAccountID(
                account.AccountID
            ),
            waterYears: this.waterYearService.getWaterYears(),
            defaultWaterYear:
                this.waterYearService.getDefaultWaterYearToDisplay(),
            parcelsToBeReconciled:
                this.accountService.getParcelsInAccountReconciliationByAccountID(
                    account.AccountID
                ),
        }).subscribe(
            ({
    ...
    public updateAnnualData() {
        ...
        forkJoin({
            parcels: this.parcelService.getParcelsByAccountID(
                this.activeAccount.AccountID,
                this.waterYearToDisplay.Year
            ),
            parcelLedgers: this.accountService.getParcelLedgersByAccountID(
                this.activeAccount.AccountID
            ),
        }).subscribe(({ parcels, parcelLedgers }) => {
            this.parcels = parcels;
            this.parcelNumbers = Array.from(
                new Set(parcels.map((x) => x.ParcelNumber))
            );
    ...
    private updateParcelLedgersAndChartDataForWaterYear(): void {
        ...
        this.waterUsages = {
            ...
            AnnualUsage: this.createWaterUsagesForYear(
                this.waterYearToDisplay.Year
            ),
        };
    ...
    public getNoTradesMessage(): string {
        if (this.tradeStatusIDs.length === 1) {
            return (
                "You have no open trades for " +
                this.waterYearToDisplay?.Year +
                ". Change the viewing year in the top of the Landowner Dashboard to view open trades for other years."
            );
        } else {
            return (
                "You have no trade activity for " +
                this.waterYearToDisplay?.Year +
                ". Change the viewing year in the top of the Landowner Dashboard to view open trades for other years."
            );
        }
    ...
    public getNoPostingsMessage(): string {
        if (this.postingStatusIDs.length === 1) {
            return (
                "You have no open postings for " +
                this.waterYearToDisplay?.Year +
                ". Change the viewing year in the top of the Landowner Dashboard to view open postings for other years."
            );
        } else {
            return (
                "You have no posting activity for " +
                this.waterYearToDisplay?.Year +
                ". Change the viewing year in the top of the Landowner Dashboard to view posting activity for other years."
            );
        }
    }
    ...
    public getPostingsForWaterYear(): Array<PostingDto> {
        return this.postings
            .filter(
                (x) =>
                    new Date(x.PostingDate).getFullYear().toString() ===
                        this.waterYearToDisplay?.Year.toString() &&
                    this.postingStatusIDs.includes(
                        x.PostingStatus.PostingStatusID
                    )
            )
            .sort((a, b) =>
    ...
    public getTradesForWaterYear(): Array<TradeWithMostRecentOfferDto> {
        return this.trades
            ? this.trades
                  .filter(
                      (x) =>
                          new Date(x.OfferDate).getFullYear().toString() ===
                              this.waterYearToDisplay?.Year.toString() &&
                          (this.tradeStatusIDs.includes(
                              x.TradeStatus.TradeStatusID
                          ) ||
                              (x.OfferStatus.OfferStatusID ===
                                  OfferStatusEnum.Accepted &&
                                  !this.isTradeCanceled(x) &&
                                  (x.BuyerRegistration.IsPending ||
                                      x.SellerRegistration.IsPending)))
                  )
                  .sort((a, b) =>
                      a.OfferDate > b.OfferDate
                          ? -1
                          : a.OfferDate < b.OfferDate
                          ? 1
                          : 0
    ...
    public doesMostRecentOfferBelongToCurrentUser(
        trade: TradeWithMostRecentOfferDto
    ): boolean {
        return (
            trade.OfferCreateAccount.AccountID === this.activeAccount.AccountID
        );
    }

    public getTradeStatus(trade: TradeWithMostRecentOfferDto): string {
        return (
            (this.doesMostRecentOfferBelongToCurrentUser(trade)
                ? "You "
                : "They ") +
            trade.TradeStatus.TradeStatusDisplayName.toLowerCase()
        );
    }
    ...
    public getTradeDescription(trade: TradeWithMostRecentOfferDto): string {
        return (
            (this.doesMostRecentOfferBelongToCurrentUser(trade)
                ? trade.OfferPostingTypeID === PostingTypeEnum.OfferToBuy
                    ? "Buying "
                    : "Selling "
                : trade.OfferPostingTypeID === PostingTypeEnum.OfferToBuy
                ? "Selling "
    ...
    public isTradeCanceled(trade: TradeWithMostRecentOfferDto) {
        return (
            trade.BuyerRegistration.IsCanceled ||
            trade.SellerRegistration.IsCanceled
        );
    }

    public isTradeRegisteredByUser(trade: TradeWithMostRecentOfferDto) {
        return (
            (trade.BuyerRegistration.IsRegistered &&
                trade.Buyer.AccountID === this.activeAccount.AccountID) ||
            (trade.SellerRegistration.IsRegistered &&
                trade.Seller.AccountID === this.activeAccount.AccountID)
        );
    }
    ...
    public getSelectedParcelIDs(): Array<number> {
        return Array.from(
            new Set(this.parcels.map((item: any) => item.ParcelID))
        );
    }
    ...
    private getParcelLedgersForWaterYear(year?: number) {
        ...
        return this.parcelLedgers.filter(
            (x) =>
                x.WaterYear == year &&
                this.parcelNumbers.includes(x.Parcel.ParcelNumber)
        );
    }

    public getAnnualWaterSupply(
        year: number,
        skipConvertToUnitsShown?: boolean
    ): number {
        let parcelLedgers = this.getWaterSupplyForWaterYear(year);
        return (
            this.getTotalTransactionAmountForParcelLedgers(
                parcelLedgers,
                skipConvertToUnitsShown
            ) ?? 0
        );
    }

    public getWaterSupplyByWaterType(waterType: WaterTypeDto): number {
        let parcelLedgers = this.getWaterSupplyForWaterYear(
            this.waterYearToDisplay.Year
        ).filter((p) => p.WaterType?.WaterTypeID === waterType.WaterTypeID);
        return (
            this.getTotalTransactionAmountForParcelLedgers(parcelLedgers) ?? 0
        );
    }

    public getTotalWaterSupplyForParcelAndYear(
        parcelID: number,
        year: number
    ): string {
        var parcelLedgersForYear = this.getWaterSupplyForWaterYear(year);
        if (parcelLedgersForYear.length > 0) {
            var parcelLedgersForYearAndParcel = parcelLedgersForYear.filter(
                (p) => p.Parcel.ParcelID == parcelID
            );

            return this.getTotalTransactionAmountForParcelLedgers(
                parcelLedgersForYearAndParcel
            ).toFixed(1);
        }
    ...
    public getWaterSupplyForWaterYear(year: number): Array<ParcelLedgerDto> {
        let parcelLedgers = this.getParcelLedgersForWaterYear(year).filter(
            (p) =>
                p.TransactionType.TransactionTypeID ===
                    TransactionTypeEnum.Supply &&
                p.ParcelLedgerEntrySourceType.ParcelLedgerEntrySourceTypeID ===
                    ParcelLedgerEntrySourceTypeEnum.Manual
        );
        return parcelLedgers;
    }

    public getTradeSalesForWaterYear(year?: number) {
        return this.getParcelLedgersForWaterYear(year).filter(
            (x) =>
                x.ParcelLedgerEntrySourceType.ParcelLedgerEntrySourceTypeID ===
                    ParcelLedgerEntrySourceTypeEnum.Trade &&
    ...
    public isWaterTransferPending(waterTransfer: WaterTransferDetailedDto) {
        return (
            !waterTransfer.BuyerRegistration.IsRegistered ||
            !waterTransfer.SellerRegistration.IsRegistered
        );
    }
    ...
    public getPurchasedWaterSupply(year?: number): number {
        return (
            this.getTotalTransactionAmountForParcelLedgers(
                this.getTradePurchasesForWaterYear(year)
            ) ?? 0
        );
    }

    public getSoldWaterSupply(
        year?: number,
        skipConvertToUnitsShown?: boolean
    ): number {
        return (
            this.getTotalTransactionAmountForParcelLedgers(
                this.getTradeSalesForWaterYear(year),
                skipConvertToUnitsShown
            ) ?? 0
        );
    }

    private getTotalTransactionAmountForParcelLedgers(
        parcelLedgersForWaterYear: ParcelLedgerDto[],
        skipConvertToUnitsShown?: boolean
    ): number {
    ...
    public getTotalSupply(): number {
        return (
            this.getAnnualWaterSupply(this.waterYearToDisplay.Year) +
            this.getPurchasedWaterSupply() +
            this.getSoldWaterSupply()
        );
    }
    ...
    public getOverconsumptionCharge(): number {
        ...
        return (
            this.getOverconsumptionAmount() *
            this.waterYearToDisplay.OverconsumptionRate
        );
    }
    ...
    public getEstimatedAvailableSupply(): number {
        let annualWaterSupply = this.getAnnualWaterSupply(
            this.waterYearToDisplay.Year
        );
    ...
    private createActivityDetailGridColDefs() {
        this.activityDetailGridColDefs = [
            {
                headerName: "APN",
                field: "Parcel.ParcelNumber",
                valueGetter: function (params: any) {
                    return {
                        LinkValue: params.data.Parcel.ParcelID,
                        LinkDisplay: params.data.Parcel.ParcelNumber,
                    };
                },
            ...
            ),
            {
                ...
                field: "TransactionType.TransactionTypeName",
            },
            {
                headerName: "Source Type",
                field: "ParcelLedgerEntrySourceType.ParcelLedgerEntrySourceTypeDisplayName",
            },
            this.utilityFunctionsService.createDecimalColumnDef(
                "Transaction Volume (ac-ft)",
                "TransactionAmount"
            ),
            this.utilityFunctionsService.createDecimalColumnDef(
                "Transaction Depth (ac-ft / ac)",
                "TransactionDepth"
            ),
            {
        ...
        if (this.includeWaterSupply()) {
            const supplyTypeColDef: ColDef = {
                ...
                valueGetter: (params) =>
                    params.data.WaterType
                        ? params.data.WaterType.WaterTypeName
                        : "-",
            };

            this.activityDetailGridColDefs.splice(
                this.supplyTypeColDefInsertIndex,
                0,
                supplyTypeColDef
            );
        }
    ...
    initializeCharts() {
        ...
        this.waterUsageChartData = {
            Year: this.waterYearToDisplay.Year,
            ChartData: this.createParcelMonthlyUsageChartData(
                this.waterYearToDisplay.Year
            ),
        };
        ...
        const waterSupplyLabel = environment.allowTrading
            ? "Annual Supply (Water Supply +/- Trades)"
            : "Annual Supply";
        if (this.includeWaterSupply()) {
            this.annualWaterSupplyChartData = this.waterYears.map((x) => {
                ...
                return {
                    ...
                    ChartData: {
                        ...
                        series: this.months.map((y) => {
                            return this.createSeriesEntry(
                                y,
                                waterSupply + purchased - sold
                            );
                        }),
                    },
                };
        ...
        this.historicCumulativeWaterUsage = new MultiSeriesEntry(
            "Average Consumption (All Years)",
            this.waterUsageOverview.Historic
        );
        this.historicAverageAnnualUsage = this.waterUsageOverview.Historic.find(
            (x) => x?.name == this.months[11]
        )?.value as number;
    }

    private createSeriesEntry(
        name: string,
        value: number,
        isEmpty?: boolean
    ): SeriesEntry {
    ...
    private createParcelMonthlyUsageChartData(
        year: number
    ): MultiSeriesEntry[] {
        ...
        for (let parcelLedger of parcelLedgers) {
            if (
                !usageByParcelAndMonth[parcelLedger.Parcel.ParcelNumber][
                    parcelLedger.WaterMonth
                ]
            ) {
                usageByParcelAndMonth[parcelLedger.Parcel.ParcelNumber][
                    parcelLedger.WaterMonth
            ...
            usageByParcelAndMonth[parcelLedger.Parcel.ParcelNumber][
                parcelLedger.WaterMonth
            ] += parcelLedger.TransactionAmount;
        }
    ...
    private createParcelUsageOverviewChartData(): WaterSupplyOverviewDto {
        let usageParcelLedgers = this.parcelLedgers.filter(
            (x) =>
                x.TransactionType.TransactionTypeID == TransactionTypeEnum.Usage
        );
        ...
        for (let waterYear of this.waterYears) {
            ...
            for (let i = 1; i < 13; i++) {
                let parcelLedgersForMonth = usageParcelLedgers.filter(
                    (x) => x.WaterYear === waterYear.Year && x.WaterMonth === i
                );
                let isEmpty = parcelLedgersForMonth.length == 0;

                if (!isEmpty) {
                    cumulativeMonthlyUsage = parcelLedgersForMonth.reduce(
                        (a, b) => {
                            return a + b.TransactionAmount;
                        },
                    ...
                    if (!historicUsageSums[i]) {
                        historicUsageSums[i] = {
                            usageSum: cumulativeMonthlyUsage,
                            monthsWithDataCount: 1,
                        };
                    } else {
                        historicUsageSums[i]["usageSum"] +=
                            cumulativeMonthlyUsage;
                        historicUsageSums[i]["monthsWithDataCount"]++;
                    }
                }
                usageByYearAndMonth[waterYear.Year][i] = isEmpty
                    ? null
                    : cumulativeMonthlyUsage;
            }
        }
        ...
        return {
            Current: this.waterYears.map((y) => {
                return {
                    ...
                    CumulativeWaterUsage: this.months.map((month, i) => {
                        let monthlyUsageValue =
                            usageByYearAndMonth[y.Year][i + 1];
                        let isEmpty = monthlyUsageValue == null;

                        return this.createSeriesEntry(
                            month,
                            isEmpty ? 0 : Math.abs(monthlyUsageValue),
                            isEmpty
                        );
                    }),
                };
            ...
            Historic: this.months.map((month, i) => {
                ...
                const seriesValue = Math.abs(
                    historicUsageSums[i + 1]["usageSum"] /
                        historicUsageSums[i + 1]["monthsWithDataCount"]
                );
    ...
    public isWaterUsageForWaterYearNullOrEmpty(): boolean {
        ...
        return this.waterUsageChartData.ChartData.every((x) =>
            x.series.every((y) => y["IsEmpty"])
        );
    }
    ...
    public getCumulativeWaterUsageForWaterYear(): SeriesEntry[] {
        ...
        let currentYearData = this.waterUsageOverview.Current.find(
            (x) => x.Year == this.waterYearToDisplay.Year
        );
        return currentYearData
            ? currentYearData.CumulativeWaterUsage
            : this.emptyCumulativeWaterUsage;
    }
    ...
    public getAnnualWaterSupplySeries(): MultiSeriesEntry {
        ...
        const annualWaterSupply = this.annualWaterSupplyChartData.find(
            (x) => x.Year == this.waterYearToDisplay.Year
        );
